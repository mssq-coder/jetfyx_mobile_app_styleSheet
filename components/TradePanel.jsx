...
const ChartTradePanelRN = ({
  ...
  const lotPresets = [0.01, 0.1, 0.5, 1.0];
    ...
    <View className="my-3">
      ...
      <View className="flex-row items-center justify-between mb-2">
        {dropdown && (
          ...
          >
            <Text
              className={`${isDark ? "text-slate-100" : "text-slate-800"} text-xs font-semibold`}
            >
              {symbol} {derivedOpen ? "▲" : "▼"}
            </Text>
          </TouchableOpacity>
        ...
        {spread && (
          <View
            className={`px-2 py-1 rounded-full ${isDark ? "bg-indigo-500/30" : "bg-indigo-500/15"} border ${isDark ? "border-indigo-400/40" : "border-indigo-400/40"}`}
          >
      ...
      {derivedOpen && (
        <View
          className={`${isDark ? "bg-slate-900/70" : "bg-white/90"} rounded-xl p-3 border ${isDark ? "border-slate-700" : "border-slate-300"} shadow-lg`}
        >
          {/* Main content grid */}
          <View className="flex-row">
            ...
            <View className="flex-1 mr-3">
              <View className="flex-row mb-2">
                <Animated.View
                  style={{
                    transform: [
                      { scale: clickedSide === "SELL" ? scaleAnim : 1 },
                    ],
                  }}
                ...
                >
                  ...
                  <View className="mt-1 flex-row justify-between items-center">
                    <Text className="text-white/80 text-[10px] font-medium tracking-wide">
                      SELL
                    </Text>
                ...
                <Animated.View
                  style={{
                    transform: [
                      { scale: clickedSide === "BUY" ? scaleAnim : 1 },
                    ],
                  }}
              ...
              <View className="flex-row flex-wrap">
                {lotPresets.map((p) => (
                  ...
                  >
                    <Text
                      className={`text-[11px] font-semibold ${Number(lot) === p ? "text-white" : isDark ? "text-slate-100" : "text-slate-800"}`}
                    >
                      {p.toFixed(2)}
                    </Text>
                  </TouchableOpacity>
            ...
            <View className="w-28">
              <View
                className={`items-center rounded-xl p-3 mb-3 border ${isDark ? "border-slate-700 bg-slate-800/50" : "border-slate-300 bg-white/70"}`}
              >
                <Text className="text-[10px] tracking-wide font-semibold text-indigo-400 mb-2">
                  LOT SIZE
                </Text>
                <View
                  className={`flex-row items-center border rounded-full px-2 py-1 mb-2 ${isDark ? "border-slate-600" : "border-slate-300"}`}
                >
                  <TouchableOpacity
                  ...
                  >
                    <Text
                      className={`${isDark ? "text-slate-50" : "text-slate-900"} text-xs font-bold`}
                    >
                      −
                    </Text>
                  </TouchableOpacity>
